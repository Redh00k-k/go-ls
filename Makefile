...
build:
ifeq ($(OS), Windows_NT)
	go build -o ${BINARY_NAME}.exe -ldflags="-s -w" -trimpath main.go print.go pattern.go utils_windows.go
...
# 	go build -o ${BINARY_NAME} -ldflags="-s -w" -trimpath main.go print.go pattern.go utils_linux.go
	...
	GOARCH=amd64 GOOS=linux go build -o ${BINARY_NAME} -ldflags="-s -w" -trimpath main.go print.go pattern.go utils_linux.go
